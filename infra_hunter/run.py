"""
Peru Job Hunter - Junior IT Infrastructure Profile
Busca puestos junior de Infraestructura TI, Sysadmin, Redes, Cloud, DevOps.

Uso:
  python -m infra_hunter.run                 # Busqueda completa con Claude
  python -m infra_hunter.run --no-claude      # Solo scraping, sin analisis
  python -m infra_hunter.run --no-batch       # Claude secuencial (sin batch API)
"""
...
logger = setup_logging("infra_hunter")
...
def main():
    parser = argparse.ArgumentParser(description="Peru Job Hunter - Junior IT Infrastructure")
